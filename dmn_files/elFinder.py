...
from dmn_utils.spizz.pytils.translit import iaaslugify
...
class connector():
	...
	_options = {
		...
		'imgLib': 'auto',
		'tmbDir': '.tmb',
        'tmbPath': '/Users/iaa/www/django/app/files/media/dmn/elf/.tmb',
		'tmbAtOnce': 5,
		'tmbSize': 48,
		'fileURL': True,
		'uploadMaxSize': 256,
	...
	def __rename(self):
		...
		if 'name' in self._request and 'current' in self._request and 'target' in self._request:
			name = iaaslugify(self._request['name'])
	...
	def __upload(self):
		...
		if 'current' in self._request:
			...
			for name, data in upFiles.iteritems():
				if name:
					total += 1
					name = iaaslugify(os.path.basename(name))
	...
	def __cdc(self, path):
		...
		for f in sorted(os.listdir(path)):
			...
			if info['mime'] == 'directory':
				if info['name'] not in self._options['iexclude']:
					dirs.append(info)
			else:
				files.append(info)
		...
		if self.__isAllowed(path, 'read'):
			for d in sorted(os.listdir(path)):
				...
				if os.path.isdir(pd) and not os.path.islink(pd) and self.__isAccepted(d):
					name = self.__checkUtf8(os.path.basename(pd))
					if name not in self._options['iexclude']:
